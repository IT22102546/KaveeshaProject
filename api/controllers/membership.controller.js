...
export const createMembership = async (req, res, next) => {
    try {
        ...
        const { name, email, adress, mobile, regNo, batch} = req.body;
        console.log(name)
        console.log(email)
        console.log(adress)
        console.log(mobile)
...
export const getAllMembership = async (req, res, next) => {
  ...
  export const acceptMembership = async (req, res, next) => {
    try {
        ...
        await sendEmail(updatedMembership.email, 'Welcome to SLQPSG', 'Your membership has been accepted.', emailContent);
